...
expectation function, and the `test_col_is_posix()` test function all
check whether one or more columns in a table is of the R `POSIXct`
date-time type. Like many of the `col_is_*()`-type functions in
**pointblank**, the only requirement is a specification of the column
names. The validation function can be used directly on a data table or
...
Each validation step or expectation will operate over a single test
unit, which is whether the column is a `POSIXct`-type column or not.
...
When `col_is_posix()` is represented in YAML (under the top-level
...
`col_is_posix()` as a validation step is expressed in R code and in the
...
following examples will validate that that column is of the `POSIXct`
and `POSIXt` classes.
...
\`col_is_posix()\` help
file.](https://raw.githubusercontent.com/rstudio/pointblank/main/images/man_col_is_posix_1.png)
