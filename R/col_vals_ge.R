...
#' expectation function, and the `test_col_vals_ge()` test function all check
...
#'   compatible column given in `vars()`. Any column values greater than or
...
#' happens. For the `col_vals_*()`-type functions, using `action_levels(warn =
#' 0.25)` or `action_levels(error = 0.25)` are good choices depending on the
...
#' `col_vals_ge()` is represented in YAML (under the top-level `steps` key as a
#' list member), the syntax closely follows the signature of the validation
#' function. Here is an example of how a complex call of `col_vals_ge()` as a
...
#' \if{html}{
#' \out{
#' `r pb_get_image_tag(file = "man_col_vals_ge_1.png")`
#' }
#' }
#'
