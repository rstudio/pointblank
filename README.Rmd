...
Next up is an example that follows the second, *agent*-less workflow (where validation step functions operate directly on data). We use the same two validation step functions as before but, this time, use them directly on the data! In this workflow, an error occur will occur if there is a single 'fail' unit:
