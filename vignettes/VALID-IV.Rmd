...
- `test_col_vals_le()`: Test whether column data are less than or equal to a specified value.
...
- `test_col_vals_ge()`: Test whether column data are greater than or equal to a specified value.
...
<button class="btn true">
<code>small_table |>
  test_col_vals_regex(vars(b), pattern = ".-[a-z]{3}.*")
</code>
</button>
...
<code>small_table |> 
  test_col_vals_gt(
    vars(z), value = vars(a),
    preconditions = \(x) x |> dplyr::mutate(z = c + e), threshold = 0.5
  )</code>
</button>
...
```{r echo=FALSE, eval=FALSE}
small_table |> test_col_vals_regex(b, pattern = ".-[a-z]{3}.*")
```
