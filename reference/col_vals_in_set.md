...
The `col_vals_in_set()` validation function, the
`expect_col_vals_in_set()` expectation function, and the
`test_col_vals_in_set()` test function all check whether column values
in a table are part of a specified `set` of values. The validation
...
- set:
  ...
  *Set of values*
  ...
  A vector of numeric or string-based elements, where column values
...
When `col_vals_in_set()` is represented in YAML (under the top-level
...
`col_vals_in_set()` as a validation step is expressed in R code and in
...
containing `low`, `mid`, and `high`. We'll determine if this validation
has any failing test units (there are 13 test units, one for each row).
...
\`col_vals_in_set()\` help
file.](https://raw.githubusercontent.com/rstudio/pointblank/main/images/man_col_vals_in_set_1.png)
