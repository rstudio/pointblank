...
#'
#' The `col_is_factor()` validation step function checks whether one or more
#' columns is of the factor type. Like many of the `col_is_*()`-type functions
...
#' operate over a single test unit, which is whether the column is a factor-type
#' column or not.
#' 
...
expect_col_is_factor <- function(object,
  ...
  testthat::expect(
    ...
    failure_message = glue::glue("The column {columns} isn't a `factor` column.")
  )
