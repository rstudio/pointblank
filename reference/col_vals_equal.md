...
The `col_vals_equal()` validation function, the
`expect_col_vals_equal()` expectation function, and the
`test_col_vals_equal()` test function all check whether column values in
a table are equal to a specified `value`. The `value` can be specified
as a single, literal value or as a column name given in
...
- value:
  ...
  *Value for comparison*
  ...
  A value used for this test of equality. This can be a single value or
  a compatible column given in
  [`vars()`](https://dplyr.tidyverse.org/reference/vars.html). Any
  column values equal to what is specified here will pass validation.
...
When `col_vals_equal()` is represented in YAML (under the top-level
...
`col_vals_equal()` as a validation step is expressed in R code and in
...
Validate that values in column `a` are all equal to the value of `5`.
...
\`col_vals_equal()\` help
file.](https://raw.githubusercontent.com/rstudio/pointblank/main/images/man_col_vals_equal_1.png)
