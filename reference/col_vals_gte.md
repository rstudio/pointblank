...
expectation function, and the `test_col_vals_gte()` test function all
...
When `col_vals_gte()` is represented in YAML (under the top-level
...
`col_vals_gte()` as a validation step is expressed in R code and in the
...
\`col_vals_gte()\` help
file.](https://raw.githubusercontent.com/rstudio/pointblank/main/images/man_col_vals_gte_1.png)
