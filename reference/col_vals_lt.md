...
expectation function, and the `test_col_vals_lt()` test function all
check whether column values in a table are *less than* a specified
...
- value:
  ...
  *Value for comparison*
  ...
  column values less than what is specified here will pass validation.
...
When `col_vals_lt()` is represented in YAML (under the top-level `steps`
...
`col_vals_lt()` as a validation step is expressed in R code and in the
...
Validate that values in column `c` are all less than the value of `5`.
...
\`col_vals_lt()\` help
file.](https://raw.githubusercontent.com/rstudio/pointblank/main/images/man_col_vals_lt_1.png)
...
The analogous function with a right-closed bound:
