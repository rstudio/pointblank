...
Thresholds will be obtained from those applied for the `error` state.
...
or the same argument of any included validation function. If `error`
thresholds are not available, then a threshold value of `1` will be used
for each generated `expect_*()` in the resulting **testthat** test file.
...
with `write_testthat_file()`. The `error` thresholds will be ported over
