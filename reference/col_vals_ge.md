...
The `col_vals_ge()` validation function, the `expect_col_vals_ge()`
expectation function, and the `test_col_vals_ge()` test function all
check whether column values in a table are *greater than or equal to* a
...
- value:
  ...
  *Value for comparison*
  ...
  column values greater than or equal to what is specified here will
  pass validation.
...
When `col_vals_ge()` is represented in YAML (under the top-level `steps`
...
`col_vals_ge()` as a validation step is expressed in R code and in the
...
Validate that values in column `a` are all greater than or equal to the
value of `5`. We'll determine if this validation has any failing test
...
\`col_vals_ge()\` help
file.](https://raw.githubusercontent.com/rstudio/pointblank/main/images/man_col_vals_ge_1.png)
...
be customized with the `actions` option.
    ...
    tbl |> col_vals_ge(columns = a, value = 5)
    #> # A tibble: 6 x 6
    #>       a     b     c d     e     f
    #>   <dbl> <dbl> <dbl> <chr> <chr> <chr>
    #> 1     5     1     1 E     A     A
...
a time. This is primarily used in **testthat** tests.
    ...
    expect_col_vals_ge(tbl, columns = a, value = 5)
...
to us.
    ...
    test_col_vals_ge(tbl, columns = a, value = 5)
    #> [1] TRUE
...
The analogous function with a left-open bound:
