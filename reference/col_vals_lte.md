...
expectation function, and the `test_col_vals_lte()` test function all
...
When `col_vals_lte()` is represented in YAML (under the top-level
...
`col_vals_lte()` as a validation step is expressed in R code and in the
...
\`col_vals_lte()\` help
file.](https://raw.githubusercontent.com/rstudio/pointblank/main/images/man_col_vals_lte_1.png)
