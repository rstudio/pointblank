...
`test_col_vals_increasing()` test function all check whether column
values in a table are increasing when moving down a table. There are
...
for allowing decreasing movements up to a certain threshold. The
...
- decreasing_tol:
  ...
  *Optional tolerance threshold for backtracking*
  ...
  values in the negative direction. By default this is `NULL` but using
  a numerical value with set the absolute threshold of negative travel
...
When `col_vals_increasing()` is represented in YAML (under the top-level
...
`col_vals_increasing()` as a validation step is expressed in R code and
...
`session_start`, which contains date-time values. Let's ensure that this
column has values that are increasing from top to bottom.
...
increasing, and, allow for repeating values (`allow_stationary` will be
set to `TRUE`). We'll determine if this validation has any failing test
units (there are 2000 test units).
...
\`col_vals_increasing()\` help
file.](https://raw.githubusercontent.com/rstudio/pointblank/main/images/man_col_vals_increasing_1.png)
