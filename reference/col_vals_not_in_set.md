...
`test_col_vals_not_in_set()` test function all check whether column
...
When `col_vals_not_in_set()` is represented in YAML (under the top-level
...
`col_vals_not_in_set()` as a validation step is expressed in R code and
...
column values are not part of a given set.
...
Validate that values in column `f` contain none of the values `lows`,
`mids`, and `highs`. We'll determine if this validation has any failing
test units (there are 13 test units, one for each row).
...
\`col_vals_not_in_set()\` help
file.](https://raw.githubusercontent.com/rstudio/pointblank/main/images/man_col_vals_not_in_set_1.png)
