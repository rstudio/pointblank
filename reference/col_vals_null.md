...
expectation function, and the `test_col_vals_null()` test function all
check whether column values in a table are `NA` values or, in the
database context, `NULL` values. The validation function can be used
...
operate over the number of test units that is equal to the number of
...
When `col_vals_null()` is represented in YAML (under the top-level
...
`col_vals_null()` as a validation step is expressed in R code and in the
...
For all examples here, we'll use a simple table with four columns: `a`,
`b`, `c`, and `d`.
...
Validate that all values in column `c` are `NA` (they would be `NULL` in
a database context, which isn't the case here). We'll determine if this
validation has any failing test units (there are 5 test units, one for
each row).
...
\`col_vals_null()\` help
file.](https://raw.githubusercontent.com/rstudio/pointblank/main/images/man_col_vals_null_1.png)
