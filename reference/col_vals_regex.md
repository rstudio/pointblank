...
- regex:
  ...
  *Regex pattern*
  ...
  A regular expression pattern to test for a match to the target column.
  Any regex matches to values in the target `columns` will pass
  validation.
...
When `col_vals_regex()` is represented in YAML (under the top-level
...
`col_vals_regex()` as a validation step is expressed in R code and in
...
The `small_table` dataset in the package has a character-based `b`
column with values that adhere to a very particular pattern. The
following examples will validate that that column abides by a regex
pattern.
...
determine if this validation has any failing test units (there are 13
...
\`col_vals_regex()\` help
file.](https://raw.githubusercontent.com/rstudio/pointblank/main/images/man_col_vals_regex_1.png)
