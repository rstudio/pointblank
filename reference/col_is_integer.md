...
The `col_is_integer()` validation function, the
`expect_col_is_integer()` expectation function, and the
`test_col_is_integer()` test function all check whether one or more
columns in a table is of the integer type. Like many of the
...
When `col_is_integer()` is represented in YAML (under the top-level
...
`col_is_integer()` as a validation step is expressed in R code and in
...
For all examples here, we'll use a simple table with a character column
(`a`) and a integer column (`b`).
...
\`col_is_integer()\` help
file.](https://raw.githubusercontent.com/rstudio/pointblank/main/images/man_col_is_integer_1.png)
