...
```{r action_levels_print_2, eval=FALSE}
al
```
...
This is a great way to test a function for use as a validation step 'action' because it will be evaluated in an environment that contains `x` with the same structure (but only if the failure condition is met).
