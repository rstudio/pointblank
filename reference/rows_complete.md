...
expectation function, and the `test_rows_complete()` test function all
check whether rows contain any `NA`/`NULL` values (optionally
constrained to a selection of specified `columns`). The validation
...
an expectation, this will operate over the number of test units that is
...
When `rows_complete()` is represented in YAML (under the top-level
...
`rows_complete()` as a validation step is expressed in R code and in the
...
are only complete rows (i.e., all rows have no `NA` values).
...
\`rows_complete()\` help
file.](https://raw.githubusercontent.com/rstudio/pointblank/main/images/man_rows_complete_1.png)
