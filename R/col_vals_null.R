...
#'
#' The `col_vals_null()` validation step function and the
#' `expect_col_vals_null()` expectation function both check whether column
#' values in a table are `NA` values or, in the database context, `NULL` values.
...
expect_col_vals_null <- function(object,
  ...
  expectation_type <- "expect_col_vals_null"
